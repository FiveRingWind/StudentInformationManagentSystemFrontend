...
const columns = [{
    ...
    render(text, record) {
        return (
            <div>
                <Change record={record} collegeid={record.collegeid}/><Divider type='vertrical'/><Delete
...
const ViewHeader = () => (
    <div className="view-header">
        <header className="title text-white">
            <h1 className="h4 text-uppercase">学院管理</h1>
            <p className="mb-0">在此页面进行对学院信息的增删改查</p>
        </header>
    </div>
);
...
class Change extends React.Component {
    ...
    handleOk = () => {
        ...
        $.ajax({
                type: "POST",
                url: weblocation + "/admin/college/edit",
                cache: false,
                crossDomain: true,
                xhrFields: {
    ...
    render() {
        return (
            <span>
                ...
                >
                    <form>
                        <ValidateForm type="text" title="学院名称" hint={this.old.name} value={this.old.name}
                                      vadlidate={(obj, value) => {
                                          this.form.name = '';
                                          if (value.length > 10) {
                                              return [false, "学院名称长度不能超过10"]
                                          }
...
class Delete extends React.Component {
    ...
    onConfirm() {
        var obj = this;
        var postbody = {
            collegeid: [obj.state.collegeid]
        }
        $.ajax({
            type: "POST",
            url: weblocation + "/admin/college/delete",
            cache: false,
            crossDomain: true,
            xhrFields: {
                withCredentials: true
    ...
    render() {
        return (
            <Popconfirm title="您是否要删除该条学院信息" okText="Yes" cancelText="No" onConfirm={() => this.onConfirm()}>
                <a>删除</a>
            </Popconfirm>
        )
    }
...
class Add extends React.Component {
    ...
    handleOk = () => {
        ...
        $.ajax({
                type: "POST",
                url: weblocation + "/admin/college/add",
                cache: false,
                crossDomain: true,
                xhrFields: {
                    withCredentials: true
            ...
            <div>
                ...
                >
                    <form>
                        ...
                        <ValidateForm type="text" title="学院名称" hint="计算机科学与工程学院" vadlidate={(obj, value) => {
                            this.form.name = '';
                            if (value.length === 0) {
                                return [false, "请输入学院名称"]
...
export default class AdminCollege extends React.Component {
    constructor(props) {
        super(props)
    ...
    fetch = (params = {}) => {
        ...
        $.ajax({
            type: "GET",
            url: weblocation + "/admin/college/view",
            cache: false,
            crossDomain: true,
            xhrFields: {
                withCredentials: true
    ...
    handleDownLoadmodle(event) {
        ...
        window.open(weblocation + "/admin/templatedownload?type=2");
    }
